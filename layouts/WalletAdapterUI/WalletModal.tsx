import { WalletName, WalletReadyState, WalletAdapterNetwork } from '@solana/wallet-adapter-base';
import { useWallet, Wallet } from '@solana/wallet-adapter-react';
import React, {
  FC, MouseEvent, MouseEventHandler, useCallback,
  useLayoutEffect, useMemo, useRef, useState, useEffect,
} from 'react';
import { createPortal } from 'react-dom';
import { useWalletModal } from '@solana/wallet-adapter-react-ui';
import { Button } from '@/layouts/WalletAdapterUI/Button';
import { WalletIcon } from '@/layouts/WalletAdapterUI/WalletIcon';
import { WalletSVG } from '@/layouts/WalletAdapterUI/WalletSVG';
import { Collapse } from '@/layouts/WalletAdapterUI/WalletCollapse';
...
const WalletListItem: FC<WalletListItemProps> = function WalletListItem({
  handleClick, tabIndex, wallet,
}) {
  return (
    <li>
...
export const WalletModal: FC<WalletModalProps> = ({ className = '', container = 'body', setNetwork }) => {
  ...
  const getStartedWallet = useMemo(() => (installedWallets.length
    ? installedWallets[0]
...
|| otherWallets[0]), [installedWallets, wallets, otherWallets]);
  ...
  useEffect(() => {
    const getNetwork = async () => {
      const res = await fetch(`/api/cache/get/${router.pathname}`);
                          ...
                          <ul className="wallet-adapter-modal-list">
                            {otherWallets.map((wallet) => (
                              <WalletListItem
                                key={wallet.adapter.name}
                                handleClick={
                                    (event) => handleWalletClick(event, wallet.adapter.name)
                                }
                                tabIndex={expanded ? 0 : -1}
                                wallet={wallet}
                              />
