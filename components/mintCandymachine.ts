...
import { sendTransactions, SequenceType } from '@/components/connection';
...
): Promise<[anchor.web3.PublicKey, number]> => anchor.web3.PublicKey.findProgramAddress(
...
export const mintOneToken = async (
  ...
  try {
    ...
    console.log('Error in mintOneToken sendTransactios: ', e);
    throw new Error(e.toString());
  }
};

export const shortenAddress = (address: string, chars = 4): string => `${address.slice(0, chars)}...${address.slice(-chars)}`;
