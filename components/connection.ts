...
export const sendTransactions = async (
  ...
  for (let i = 0; i < instructionSet.length; i += 1) {
  ...
  for (let i = 0; i < signedTxns.length; i += 1) {
    ...
    if (sequenceType !== SequenceType.Parallel) {
      try {
        ...
        if (e.toString().includes('0x1778') || e.toString().includes('0x135') || e.toString().includes('6008')) {
...
export async function sendTransactionsWithManualRetry(
  connection: Connection,
  wallet: any,
  instructionsParam: TransactionInstruction[][],
  signers: Keypair[][],
  ...
  let instructions = instructionsParam.filter((instr, i) => {
    if (instr.length > 0) {
      return true;
    }
  ...
  while (stopPoint < instructions.length && tries < 3) {
    ...
    try {
      ...
      console.error('Error in connection line 481: ', e);
